...
Bit-exactness against the transient-transpose path is covered by the fused-MoE
parity tests.
"""
...
_STRIDED_FALLBACK_HINT = (
    "set XORL_MOE_SGLANG_FUSED_EXPERTS_WEIGHT_MODE=transient to fall back to the transpose-copy path"
)
...
def _fm():
    ...
    try:
        ...
        raise ImportError(
            "XORL_MOE_SGLANG_FUSED_EXPERTS_WEIGHT_MODE=strided (the default) could not import "
...
def fused_experts_impl_strided(
    ...
    try:
        ...
        raise RuntimeError(
            "XORL_MOE_SGLANG_FUSED_EXPERTS_WEIGHT_MODE=strided (the default) could not bind the vendored "
