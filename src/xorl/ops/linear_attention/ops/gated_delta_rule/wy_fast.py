...
from xorl.ops.linear_attention.utils import IS_NVIDIA_BLACKWELL, IS_NVIDIA_HOPPER, autotune_cache_kwargs, check_shared_mem
...
# SM90-class GPUs can hit illegal memory access in GDN backward autotuning
# with the 4-warps/2-stages variant on real Qwen3.5 shapes (e.g. T=4096, H=4, D=128).
# Keep the bad candidate out until the Triton codegen issue is resolved upstream.
PREPARE_WY_REPR_BWD_AUTOTUNE_CONFIGS = [
    triton.Config({}, num_warps=num_warps, num_stages=num_stages)
    for num_warps in [2, 4]
    for num_stages in [2, 3, 4]
    if not (IS_NVIDIA_HOPPER and num_warps == 4 and num_stages == 2)
]
...
@triton.autotune(
    configs=PREPARE_WY_REPR_BWD_AUTOTUNE_CONFIGS,
    key=['H', 'K', 'V', 'BT', 'BK', 'BV', 'IS_VARLEN'],
    **autotune_cache_kwargs,
)
@triton.jit(do_not_specialize=['T'])
