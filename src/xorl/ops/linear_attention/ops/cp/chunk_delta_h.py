...
def merge_fwd_bwd_kernel(
    ...
    if INTRACARD_MODE:
        ...
        i_h = tl.program_id(1)
        # tl.cast (not .to): under torch.compile's identify_mutated_tensors tracing,
        # do_not_specialize scalars arrive as plain Python ints, which have no .to —
        # the tracing CompilationError makes Dynamo assume every input is mutated and
        # clone all kernel inputs each call. tl.cast handles both ints and scalars.
        num_ranks = tl.cast(pre_or_post_num_ranks, tl.int32)
        h += i_h * K * V
        ag_hm += i_h * K * (K + V)
        stride = H * K * (K + V)
        b_h = tl.zeros([BK, BV], dtype=tl.float32)
