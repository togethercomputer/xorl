...
def validate_chain_receipt(receipt: ChainReceipt, *, state: torch.Tensor) -> None:
    ...
    if receipt.state_dtype != torch.float32 or state.dtype != torch.float32:
        raise ChainReceiptError(
            f"chain state must be fp32 end-to-end, got receipt={receipt.state_dtype}, tensor={state.dtype}",
        )
...
def chain_pre_scan_initial_state(
    *,
    ...
    """
    initial_state = torch.zeros(num_seqs, heads, key_dim, value_dim, dtype=torch.float32, device=device)
...
def chain_recv_state(
    ...
    *,
    ...
    _record_engagement(
        f"{tag_prefix}:recv",
        f"rank {receipt.receiver_rank} <- {src}, state {tuple(shape)} fp32, in-doc offset {in_doc_offset}",
    )
