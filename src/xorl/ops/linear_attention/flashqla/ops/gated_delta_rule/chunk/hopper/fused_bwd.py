...
from xorl.ops.linear_attention.flashqla.utils import prepare_chunk_offsets
            ...
            T.annotate_layout(
                {
                    ...
                    tmp_shared_1_3: tilelang.layout.make_swizzled_layout(tmp_shared_1_3),
            ...
            if tx < 128:
                ...
                for j_s, j_k in T.Parallel(block_S, DK):
                    if seq_start_idx + j_s < seq_end_idx:
                        dk[batch_idx, seq_start_idx + j_s, bh, j_k] = dk_fragment[j_s, j_k]
                ...
                for i_s in T.serial(num_iters):
                    ...
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        if j_s >= j_t:
                            mask_fragment[j_s, j_t] = T.exp2(mask_fragment[j_s, j_t] * 1.442695)
                        else:
                ...
                if tx < 384 + 32:
                    for i_s in T.serial(num_iters - 1):
                        ...
                        T.barrier_wait(bar_05, (i_s + 0) % 2)
                        T.copy(v[batch_idx, left:right, bh, 0:DV], v_shared, prefer_instruction="tma")

                        T.barrier_wait(bar_07, (i_s + 0) % 2)
                        T.copy(k[batch_idx, left:right, bhg, 0:DK], k_shared, prefer_instruction="tma")

                        T.barrier_wait(bar_10, (i_s + 0) % 2)
                        T.copy(q[batch_idx, left:right, bhg, 0:DK], q_shared, prefer_instruction="tma")
                    ...
                    for i_s in T.serial(num_iters - 1):
                        ...
                        T.barrier_wait(bar_14, (i_s + 0) % 2)
                        T.copy(a[batch_idx, left:right, bh, 0:block_S], a_shared, prefer_instruction="tma")

                        T.copy(do[batch_idx, left:right, bh, 0:DV], do_shared, prefer_instruction="tma")
