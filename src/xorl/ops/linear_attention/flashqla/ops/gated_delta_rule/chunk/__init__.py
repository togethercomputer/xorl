...
if tilelang.contrib.nvcc.get_target_compute_version() == "9.0":
    from .hopper import fused_gdr_bwd, fused_gdr_fwd, fused_gdr_h, kkt_solve  # noqa: F401
else:
    raise ValueError("FlashQLA now support sm90 only.")
from .cp_context import intra_card_cp_preprocess


def robust_kkt_solve(
    k: torch.Tensor,
    b: torch.Tensor,
    chunk_size: int = 64,
    cu_seqlens: torch.LongTensor | None = None,
):
    """Numerically-robust replacement for FlashQLA's ``kkt_solve``.

    FlashQLA's bespoke block-recursive triangular inverse (``hopper/kkt_solve.py``)
    produces NaN at large ``num_chunks*num_heads`` (e.g. H>=24 at seq>=4096) -- the
    production Qwen3.5/3.6-35B-A3B GDN shape -- because its Schur-combine runs through
    Hopper TF32 MMA and the inverse explodes. It computes
    ``(I + StrictLower(b * K @ K^T))^-1`` with no decay folded. FLA's chunk-KKT +
    triangular solve computes the *same* inverse robustly (cos 0.99999 vs reference),
    with the identical ``[B, T, H, chunk_size]`` unit-lower-triangular bf16 layout, so
    it is a drop-in for ``fused_gdr_fwd``/``fused_gdr_h``'s ``a`` input.
    """
    from xorl.ops.linear_attention.ops.common.chunk_scaled_dot_kkt import chunk_scaled_dot_kkt_fwd  # noqa: PLC0415
    from xorl.ops.linear_attention.ops.utils import solve_tril  # noqa: PLC0415

    A = chunk_scaled_dot_kkt_fwd(k=k, g=None, beta=b, cu_seqlens=cu_seqlens, output_dtype=torch.float32)
    return solve_tril(A=A, cu_seqlens=cu_seqlens, output_dtype=k.dtype)
...
def chunk_gated_delta_rule_fwd(
    ...
    if auto_cp:
        initial_state, cu_seqlens, cp_seq_map, raw_cu_seqlens = intra_card_cp_preprocess(
            k=k,
            v=v,
            a=A,
            g=g,
