...
def tilelang_prepare_h(
    ...
    def tilelang_prepare_h_kernel(
        ...
        with T.Kernel(batch_size * H, threads=512) as (bbh,):
            ...
            num_iters = num_warmup_chunks[bb, bh] if is_cp else T.ceildiv(seq_end_idx - seq_start_idx, block_S)
            ...
            if tx < 128:
                ...
                for i_s in T.serial(num_iters):
                    ...
                    for j_s in T.Parallel(block_S):
                        g_rev_exp_shared[j_s] = T.exp2((g_last_local_Y[0] - g_shared[i_s % num_stages, j_s]) * 1.442695)
                    ...
                    for j_s, j_v in T.Parallel(block_S, DV):
                        y_fragment[j_s, j_v] -= v_shared[i_s % num_stages, j_s, j_v] * g_rev_exp_shared[j_s]
                    # S2[2] Y
                        ...
                        if right <= seq_end_idx:
                            ...
                            for j_s in T.Parallel(block_S):
                                if left + j_s < seq_end_idx:
                                    ...
                                    g_shared[i_s % num_stages, j_s] = g[batch_idx, seq_end_idx - 1, bh]
                        # Load beta
                        if right <= seq_end_idx:
                            for j_s in T.Parallel(block_S):
...
def fused_gdr_h(
    ...
    if cu_seqlens is None:
        ...
        if num_warmup_chunks is None:
            num_warmup_chunks = torch.empty((real_batch_size, H), dtype=cu_seqlens.dtype, device=k.device)
            is_cp = False
        else:
            is_cp = True
