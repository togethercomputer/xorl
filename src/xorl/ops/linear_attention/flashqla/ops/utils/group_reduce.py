...
def tilelang_group_reduce_vector(
    ...
    def tilelang_group_reduce_vector_kernel(
        ...
        with T.Kernel(tilelang.cdiv(num_tokens, block_size), Hg, batch_size, threads=128) as (bt, bhg, bb):
...
def group_reduce_vector(
    ...
    result = torch.empty((batch_size, num_tokens, Hg, K), dtype=buffer.dtype, device=buffer.device)
