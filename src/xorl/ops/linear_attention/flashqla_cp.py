...
class ChunkGatedDeltaRuleCPFunction(torch.autograd.Function):
    ...
    def backward(ctx, do: torch.Tensor, _dht: torch.Tensor):
        ...
        dq, dk, dv, db, dg, dh0 = _flashqla_cp_bwd(
            q=q,
            k=k,
            v=v,
            g_fla=g_fla,
        ...
        # CP forbids a caller-provided initial state. ``dh0`` is therefore the
        # derivative of the materialized cross-rank boundary, not of an autograd
        # input. Its contribution to earlier ranks is already represented by the
        # analytical ``dhm``/``dht`` reverse merge above, as in the FLA CP path;
        # returning or reapplying it here would double-count that boundary.
        del dh0
