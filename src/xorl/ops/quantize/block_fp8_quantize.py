...
_SCALED_MM_AUTO_FALLBACK_WARNED = False


def _warn_scaled_mm_auto_fallback(exc: RuntimeError) -> None:
    global _SCALED_MM_AUTO_FALLBACK_WARNED
    if _SCALED_MM_AUTO_FALLBACK_WARNED:
        return
    _SCALED_MM_AUTO_FALLBACK_WARNED = True
    warnings.warn(
        f"torch._scaled_mm failed in the automatic FP8 GEMM path; falling back to Triton: {exc}",
        RuntimeWarning,
        stacklevel=3,
    )
    ...
    if use_torch_scaled_mm and backend in {"torch_scaled_mm", "auto"}:
        try:
            ...
            if backend == "torch_scaled_mm":
                raise
            _warn_scaled_mm_auto_fallback(exc)
