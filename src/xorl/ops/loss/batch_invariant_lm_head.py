...
def _tp_collective_layout(
    ...
    *,
    ...
    if not dist.is_available() or not dist.is_initialized():
        raise RuntimeError("ce_mode='batch_invariant' tensor parallelism requires initialized torch.distributed")
    ...
    if bool((hidden_sizes != hidden_sizes[0]).any().item()):
        raise ValueError(f"batch_invariant TP hidden widths differ across ranks: {hidden_sizes.cpu().tolist()}")
    head_families = program_flags >> 2
    if bool((head_families != head_families[0]).any().item()):
        raise ValueError(
            f"batch_invariant TP ranks must use the same head-family program, got {head_families.cpu().tolist()}"
        )
    vocab_sizes = tuple(int(value) for value in layout[:, 1].cpu().tolist())
    if any(size <= 0 for size in vocab_sizes):
        raise ValueError(f"batch_invariant TP requires a non-empty vocabulary shard on every rank, got {vocab_sizes}")
...
class _BatchInvariantVocabParallelPerTokenCE(torch.autograd.Function):
...
class _BatchInvariantLmHeadPerTokenCE(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        hidden,
...
def batch_invariant_per_token_ce(
    ...
    if not hidden_states.is_cuda:
        raise ValueError("ce_mode='batch_invariant' requires CUDA tensors")
    if hidden_states.dtype != torch.bfloat16 or weight.dtype != torch.bfloat16:
        raise ValueError("ce_mode='batch_invariant' requires bf16 hidden states and lm-head weight")
    valid_mask = labels != ignore_index
    labels_safe = torch.where(valid_mask, labels, torch.zeros_like(labels))
    if isinstance(temperature, torch.Tensor):
        if temperature.dtype is not torch.float32:
            raise TypeError("ce_mode='batch_invariant' requires per-row FP32 temperature")
        if temperature.device != hidden_states.device or tuple(temperature.shape) != (hidden_states.shape[0],):
            raise ValueError("ce_mode='batch_invariant' requires temperature aligned one-to-one with hidden-state rows")
        if not temperature.is_contiguous() or temperature.requires_grad:
            raise ValueError("ce_mode='batch_invariant' requires contiguous, non-differentiable temperature metadata")
        torch._assert_async(
            (torch.isfinite(temperature) & (temperature > 0)).all(),
            "ce_mode='batch_invariant' requires finite temperature > 0",
        )
        ...
        if not math.isfinite(temperature) or temperature <= 0:
            raise ValueError("ce_mode='batch_invariant' requires finite temperature > 0")
    ...
    if (top_ks is None, top_ps is None, min_ps is None).count(True) not in (0, 3):
        raise ValueError("ce_mode='batch_invariant' requires all or none of top-k/top-p/min-p row metadata")
    return _BatchInvariantLmHeadPerTokenCE.apply(
        hidden_states,
        weight,
        labels_safe,
        valid_mask,
...
def batch_invariant_vocab_parallel_per_token_ce(
    hidden_states: torch.Tensor,
    local_weight: torch.Tensor,
    labels: torch.Tensor,
    ...
    if not hidden_states.is_cuda or not local_weight.is_cuda:
        raise ValueError("ce_mode='batch_invariant' tensor parallelism requires CUDA tensors")
    if hidden_states.ndim != 2 or local_weight.ndim != 2:
        raise ValueError("ce_mode='batch_invariant' tensor parallelism requires two-dimensional hidden and weight")
    if hidden_states.shape[1] != local_weight.shape[1]:
        raise ValueError("ce_mode='batch_invariant' tensor parallelism received mismatched hidden dimensions")
    if hidden_states.dtype is not torch.bfloat16 or local_weight.dtype is not torch.bfloat16:
        raise ValueError("ce_mode='batch_invariant' tensor parallelism requires bf16 hidden states and lm-head weight")
    if labels.ndim != 1 or labels.shape[0] != hidden_states.shape[0]:
        raise ValueError("ce_mode='batch_invariant' tensor parallelism requires one label per local hidden row")
    ...
    if isinstance(temperature, torch.Tensor):
        if temperature.dtype is not torch.float32:
            raise TypeError("ce_mode='batch_invariant' requires per-row FP32 temperature")
        if temperature.device != hidden_states.device or tuple(temperature.shape) != (hidden_states.shape[0],):
            raise ValueError("ce_mode='batch_invariant' requires temperature aligned one-to-one with local hidden rows")
        if not temperature.is_contiguous() or temperature.requires_grad:
            raise ValueError("ce_mode='batch_invariant' requires contiguous, non-differentiable temperature metadata")
        torch._assert_async(
            (torch.isfinite(temperature) & (temperature > 0)).all(),
    ...
    for name, value in (("top_ks", top_ks), ("top_ps", top_ps), ("min_ps", min_ps)):
        if value is not None and (
            ...
            raise ValueError(f"ce_mode='batch_invariant' requires contiguous local-row {name} metadata")

    return _BatchInvariantVocabParallelPerTokenCE.apply(
        hidden_states,
        local_weight,
        labels_safe,
        valid_mask,
