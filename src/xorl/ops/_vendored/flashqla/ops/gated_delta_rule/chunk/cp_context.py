...
from xorl.ops.linear_attention.flashqla.utils import tensor_cache


if tilelang.contrib.nvcc.get_target_compute_version() == "9.0":
    from .hopper import correct_initial_states, fused_gdr_h, get_warmup_chunks
else:
...
def _create_cu_seqlens(
    ...
    return torch.arange((batch_size + 1), dtype=torch.int32, device=f"cuda:{device_idx}") * num_tokens
...
def _calc_cp_seqs(
    ...
    max_local_chunks = 2 ** round(math.log2(math.sqrt(H * sum(num_chunks) / MULTI_PROCESSOR_COUNT) * 3))
    ...
    if use_cp:
        cp_cu_seqlens = torch.tensor(cp_cu_seqlens, dtype=seqlen_dtype, device=device, requires_grad=False)
        seq_map_c2r = torch.tensor(seq_map_c2r, dtype=seqlen_dtype, device=device)
        seq_map_r2c = torch.tensor(seq_map_r2c, dtype=seqlen_dtype, device=device, requires_grad=False)
        ht_mask = torch.tensor(ht_mask, dtype=torch.bool, device=device, requires_grad=False)
    else:
