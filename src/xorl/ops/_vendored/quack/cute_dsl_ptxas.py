...
def _get_ptx(compiled_func) -> tuple[str, Path] | None:
    ...
    # Strategy 1: match by filename, but still verify the exported entry point.
    ...
    if filename_matches:
        ...
        for ptx_path in filename_matches:
            ...
            if content is None or not entry_pattern.search(content):
                continue
...
def _compile_ptx(ptx_path: Path, ptx_content: str) -> bytes:
    ...
    with tempfile.NamedTemporaryFile(
        dir=ptx_path.parent,
        prefix=f".{ptx_path.stem}.",
        suffix=".cubin.tmp",
        delete=False,
    ) as cubin_file:
        cubin_tmp = Path(cubin_file.name)
    try:
        ptxas_path = _validated_ptxas_path()
        timeout_s = float(os.environ.get("CUTE_DSL_PTXAS_TIMEOUT_S", "300"))
        if timeout_s <= 0:
            raise ValueError("CUTE_DSL_PTXAS_TIMEOUT_S must be positive")
