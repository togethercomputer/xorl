...
def _mark_owned(path: str) -> None:
    with open(os.path.join(path, OWNED_SENTINEL), "w") as f:
        f.write("created by xorl.ops.kernel_config_pin; safe for it to replace\n")
...
def seed_exact_kernel_config_pin(pin_dir: str, *, source_cache: str | None = None) -> dict:
    ...
    if not os.path.isdir(parent):
        raise KernelConfigPinError(
            f"pin directory parent {parent!r} does not exist; refusing to create a pin "
            "at an implausible location",
        )
    os.makedirs(pin_dir, exist_ok=True)
    cache_src = os.path.realpath(
        source_cache
        or os.environ.get("TRITON_CACHE_DIR", os.path.expanduser("~/.triton/cache"))
    )
