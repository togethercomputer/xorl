...
def validate_native_fp8_dcp_checkpoint(
    ...
    *,
    ...
    native_keys = [
        ...
        if name.startswith(state_prefix) and ("packed_weight_f32" in name or name.endswith("weight_scale_inv"))
    ]
    return {
        "contract_version": NATIVE_BLOCK_FP8_CONTRACT_VERSION,
        "checkpoint_path": checkpoint_path,
        "state_prefix": state_prefix,
        "validated_native_keys": tuple(sorted(native_keys)),
        "metadata_gate": "pass",
    }
...
class NativeBlockFP8Linear(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # A device-side counter is safe to capture in a CUDA graph.  It proves
        # kernel engagement across graph replays without Python mutation in
        # forward.  Capture-vs-replay launch counts remain an external runner
        # receipt because a captured kernel cannot identify its launch phase.
    ...
    def engagement_receipt(self) -> dict[str, Any]:
        return {
            "family": "native_block_fp8_linear",
            "contract_version": self.contract_version,
            "engagement_count": int(self._engagement_count.item()),
            "kernel": "sglang.srt.layers.quantization.fp8_utils.triton_w8a8_block_fp8_linear",
            "weight_shape": (self.out_features, self.in_features),
            "scale_shape": tuple(self.weight_scale_inv.shape),
