...
# Rule B: golden-value gates pin the bits in tests/ops/test_bi_families_v2.py.
#
# Status: DEFAULT ON inside an engaged BI contract lane since the N6 re-anchor.
# Stock lanes do not call these trees. Setting either XORL_FAMILIES_V2=0 or
# SGLANG_FAMILIES_V2=0 is a paired rollback kill switch; a mixed engine setting
# invalidates the contract. v1 kernels stay available.
...
def families_v2_enabled() -> bool:
    """families-v2 is DEFAULT ON for the contract lanes (the N6 re-anchor);
    ...
    return not any(os.getenv(v, "1").lower() in ("0", "false", "no") for v in FAMILIES_V2_ENV_VARS)
...
def _qk_norm_v2_kernel(
    ...
    tl.store(out_ptr + out_base[:, None] + d[None, :], y.to(out_ptr.dtype.element_ty), mask=mask)
...
def rms_norm_v2(
    ...
    *,
    ...
    assert x.is_cuda
    if zero_centered:
        assert residual is None, "zero-centered exists in no-residual form only (v1 rule kept)"
    rows, H = x.shape
...
_HEAD_V2_FALLBACK = {"BLOCK_SIZE_M": 64, "GROUP_SIZE_M": 8, "num_stages": 2, "num_warps": 4}
...
def _head_v2_gemm_stats_kernel(
    ...
    for tile_id in tl.range(start_pid, num_tiles, NUM_SMS, flatten=True):
        ...
        if STORE_LOGITS:
            ...
            tl.store(c_ptrs, accumulator.to(tl.float32), mask=row_mask[:, None] & col_mask[None, :])
...
def _head_v2_stats_from_logits_kernel(
    ...
    tl.store(m_out_ptr + row * stride_mm + tile + tl.arange(0, 1), m_tile + tl.zeros((1,), dtype=tl.float32))
...
def _rms_norm_v2_partials_kernel(
    ...
    if HAS_RESIDUAL:
        r = tl.load(res_ptr + row * stride_res + cols, mask=mask, other=0.0).to(tl.float32)
        s = _rtne_bf16(x + r)
        tl.store(res_out_ptr + row * stride_res_out + cols, s.to(res_out_ptr.dtype.element_ty), mask=mask)
    else:
        s = x
