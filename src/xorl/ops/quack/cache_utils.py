...
def _encode_cache_key(value: object) -> bytes:
    """Encode the small, immutable JIT-key surface without invoking object hooks."""
    ...
    if isinstance(value, IntEnum):
        ...
        module = type.__getattribute__(enum_type, "__module__")
        qualname = type.__getattribute__(enum_type, "__qualname__")
        return _encode_cache_key(("enum", module, qualname, int.__repr__(value)))
    ...
    if value_type in {tuple, list}:
        ...
        parts = [tag, len(value).to_bytes(8, "big")]
        for item in value:
            encoded = _encode_cache_key(item)
            parts.extend((len(encoded).to_bytes(8, "big"), encoded))
        return b"".join(parts)
    if isinstance(value, type):
        # CuTe dtype arguments are class objects. Bypass their metaclass hooks
        # and admit only the runtime namespace used by Quack's public wrappers.
        module = type.__getattribute__(value, "__module__")
        qualname = type.__getattribute__(value, "__qualname__")
        if not isinstance(module, str) or not module.startswith("cutlass.") or not isinstance(qualname, str):
            raise TypeError(f"unsupported Quack cache-key type object: {module}.{qualname}")
        return _encode_cache_key(("type", module, qualname))
    raise TypeError(f"unsupported Quack cache-key value: {value_type.__module__}.{value_type.__qualname__}")


def _key_to_hash(key: tuple) -> str:
    return hashlib.sha256(_encode_cache_key(key)).hexdigest()
