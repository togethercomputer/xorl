# ruff: noqa
# Vendored verbatim from https://github.com/tile-ai/tilelang/blob/4956b5835fa554af6c03d4a6289cad44bf310869/examples/dsa_sparse_finetune/indexer_bwd.py
# Lives in tilelang's `examples/` tree, not the installable package — no Python import path. Replace with `from tilelang...` once tile-ai/tilelang exposes these kernels.
...
def indexer_bwd_interface(
    ...
    # block_I=64, num_threads=256 is ~14% faster than the upstream defaults
    # (block_I=32, num_threads=128) at GLM-5.1 indexer shape on H100 (2.05 ms
    # vs 2.37 ms at S=2048, S_kv=32768, heads=32, dim=128, topk=2048).
    tl_indexer_bwd_impl(head_num, head_dim, k_top, block_I=64, num_threads=256)(
        index_q.contiguous(),
        index_k.contiguous(),
