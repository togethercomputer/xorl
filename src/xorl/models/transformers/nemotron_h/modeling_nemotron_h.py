...
class NemotronHForCausalLM(NemotronHPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """Return decoder hidden states under XoRL's causal-LM model contract.

        XoRL applies ``lm_head`` in the centralized loss/output stage (and as a
        separate pipeline-plan node), so model-family wrappers intentionally do
        not materialize vocabulary logits here. This matches Qwen and the other
        XoRL ``ForCausalLM`` implementations.
        """
