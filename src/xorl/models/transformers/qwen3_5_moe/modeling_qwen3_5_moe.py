...
class Qwen3_5MoeSparseMoeBlock(MoEBlock):
    ...
    def _ep_combine_native(
        ...
        (all-to-all, never NCCL-summed) and each rank applies the shared
        adjacent-pair BF16 tree to its own tokens' logical contributions.
        Forward bits match the serving engine; backward uses stock numerics throughout
        ...
        if not ps.ep_enabled:
            raise RuntimeError("Qwen3.5-MoE canonical combine requires trainer EP mirroring the serving EP size")
        if not hasattr(self.shared_expert, "gate_up_proj"):
            raise NotImplementedError("Qwen3.5-MoE canonical combine requires the fused shared-expert gate_up_proj")
        ...
        if inter % ep_size != 0:
            raise ValueError(
                f"Qwen3.5-MoE canonical combine: shared_expert intermediate_size={inter} "
        ...
        if e_local * ep_size != self.experts.num_experts:
            raise RuntimeError(
                f"Qwen3.5-MoE canonical combine: local expert slice {e_local} x ep_size {ep_size} "
        ...
        # 3./4. raw exchange + canonical fold (autograd reverses the exchange)
