...
def _single_adapter_lm_head_batch_info(
    device_index: int, rows: int, rank: int = 1, scaling: float = 1.0
) -> Any:
...
class Glm52ExactTP16LmHeadSelectedLogprob(nn.Module):
    ...
    def _exact_local_logits(
        ...
        batch_info = _single_adapter_lm_head_batch_info(
            hidden_2d.device.index, rows, self.max_lora_rank, self.scaling
        )
