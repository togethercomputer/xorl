...
def glm52_exact_lora_scaling(rank: int, alpha: int) -> float:
    if isinstance(rank, bool) or not isinstance(rank, int) or rank <= 0:
        raise ValueError(f"GLM-5.2 exact active LoRA requires a positive integer rank; got rank={rank!r}")
    if isinstance(alpha, bool) or not isinstance(alpha, int) or alpha <= 0:
        raise ValueError(f"GLM-5.2 exact active LoRA requires a positive integer alpha; got alpha={alpha!r}")
