...
def _load_sparse_selection() -> NativeScoreKernel:
    ...
    sampler_version = getattr(module, "GLM52_SELECTOR_CONTRACT_VERSION", None)
    if sampler_version != GLM52_SELECTOR_VERSION:
        raise RuntimeError(
            "GLM-5.2 sparse selector contract mismatch: "
            f"trainer={GLM52_SELECTOR_VERSION!r}, sampler={sampler_version!r}"
        )
