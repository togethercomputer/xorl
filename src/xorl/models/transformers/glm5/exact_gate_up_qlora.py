    ...
    def _surrogate_vjp(
        ...
        *,
        ...
        with torch.enable_grad(), torch.autocast(device_type=input.device.type, enabled=False):
            ...
            gate_output = self.scaling * F.linear(
                F.linear(reference_gate_input, reference_gate_A), reference_gate_B
            )
            up_output = self.scaling * F.linear(
                F.linear(reference_up_input, reference_up_A), reference_up_B
            )
