...
class MoEExperts(nn.Module):
    ...
    def sglang_ep_native_routed_partial(
        ...
        """
        if not self.gated:
            raise NotImplementedError("Qwen3.5-MoE canonical combine supports gated MoE experts only")
        if self.down_bias is not None or self.gate_up_bias is not None:
            raise NotImplementedError("Qwen3.5-MoE canonical combine does not support expert biases")
        ...
        if needs_grad:
            if self.hidden_act not in {"silu", "gelu_tanh"}:
                raise NotImplementedError(
                    "Qwen3.5-MoE canonical-combine training supports silu/gelu_tanh only "
