...
class CanonicalRouteMetadata:
    ...
    def validate(self) -> None:
        ...
        if weights.dtype is not torch.float32:
            raise CanonicalCombineError(f"topk_weights must be FP32 (weights are applied at combine), got {weights.dtype}")
...
def validate_transport_receipt(
    ...
    token_index = (
        torch.arange(metadata.num_tokens, device=mapping.device).unsqueeze(1).expand_as(mapping)
    )
    ...
    if bool(torch.any(claimed_experts != metadata.topk_ids[active].to(torch.int64))):
        raise TransportReceiptError("receive-layout validation failed: a delivered row's expert id disagrees with its slot")
    if bool(torch.any(claimed_tokens != token_index[active].to(torch.int64))):
        raise TransportReceiptError("receive-layout validation failed: a delivered row's source token disagrees with its slot")

    real_rows = receipt.row_source_tokens.to(torch.int64) >= 0
    if bool(torch.any(claims[real_rows] != 1)):
        raise TransportReceiptError("a delivered real row is claimed by no slot (a contribution would be dropped silently)")
...
def _combine_forward_reference(
    ...
    for k in range(topk):
        ...
        update = (
            acc.to(torch.float64) + rows * topk_weights[:, k].to(torch.float64).unsqueeze(1)
        ).to(torch.float32)
...
class _CanonicalCombineFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        slot_tokens = (
            torch.arange(active.shape[0], device=active.device)
            .unsqueeze(1)
            .expand_as(active)
            .reshape(-1)[active_flat]
        )
