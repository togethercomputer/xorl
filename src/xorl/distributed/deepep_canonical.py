...
def _pad_cols_to_alignment(payload: torch.Tensor) -> torch.Tensor:
    ...
    pad = torch.full((payload.shape[0], target - width), float("nan"), dtype=payload.dtype, device=payload.device)
...
def dispatch_primary(
    buffer, hidden: torch.Tensor, topk_weights: torch.Tensor, topk_ids: torch.Tensor, num_experts: int
):
...
def dispatch_slot_level(
    ...
    active = topk_ids >= 0
    slot_tokens = torch.arange(topk_ids.shape[0], device=topk_ids.device).unsqueeze(1).expand_as(topk_ids)[active]
    slot_ks = torch.arange(topk_ids.shape[1], device=topk_ids.device).unsqueeze(0).expand_as(topk_ids)[active]
...
def validate_expert_receive_alignment(
    *,
    ...
    if not torch.equal(expected_weights.view(torch.int32), got_weights.view(torch.int32)):
        raise DeepEPReceiveAlignmentError("FP32 routing-weight bytes differ between primary and slot-level receives")
    ...
    if bool(torch.any(ann_experts != expected_global)):
        raise DeepEPReceiveAlignmentError("transported global expert ids disagree with the local receive layout")
...
def build_owner_receipt(
    ...
    if returned.shape[0] != plan.num_slots:
        raise CanonicalCombineError(f"return transport delivered {returned.shape[0]} rows, expected {plan.num_slots}")
    ...
    if pad.numel() and not bool(torch.isnan(pad.float()).all()):
        raise CanonicalCombineError("return-path alignment padding arrived non-NaN: dead bytes were written in transit")
