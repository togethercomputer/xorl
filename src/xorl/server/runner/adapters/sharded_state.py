...
def deterministic_local_initialization(
    ...
    *,
    ...
    if is_lora_b:
        ...
        for dim, (size, start, logical_stride) in enumerate(
            zip(shape, starts, strides, strict=True)
        ):
            view_shape = [1] * len(shape)
            view_shape[dim] = size
            linear = linear + (
                torch.arange(size, dtype=torch.int64).reshape(view_shape) + start
            ) * logical_stride
        seed = stable_seed(base_seed, session_identity, layout.fqn)
        ...
        normal = torch.sqrt(-2.0 * torch.log(uniform1)) * torch.cos(
