    ...
    *,
    ...
    if server.default_session_spec is None:
        ...
        if raw_lora_config or raw_optimizer_config or raw_zorl_config:
            raise ValueError(
                "Per-session LoRA or optimizer overrides are not supported in "
                "full-weight mode; ZORL overrides are also unsupported."
            )
        normalized_spec = {
...
@router.post("/api/v1/zorl/start_generation", response_model=UntypedAPIFuture, tags=["ZORL"])
...
@router.post("/api/v1/zorl/apply_rewards", response_model=UntypedAPIFuture, tags=["ZORL"])
async def apply_zorl_rewards_endpoint(request: ZORLApplyRewardsRequest, server=Depends(require_api_server)):
    """Apply externally aggregated rewards for an active ZORL generation."""
    return await server.submit_apply_zorl_rewards_async(request)


@router.post("/api/v1/zorl/abort_generation", response_model=UntypedAPIFuture, tags=["ZORL"])
async def abort_zorl_generation_endpoint(request: ZORLAbortGenerationRequest, server=Depends(require_api_server)):
    """Abort an active ZORL generation without updating the parent."""
    return await server.submit_abort_zorl_generation_async(request)


@router.post("/api/v1/zorl/reset_session", response_model=UntypedAPIFuture, tags=["ZORL"])
async def reset_zorl_session_endpoint(request: ZORLResetSessionRequest, server=Depends(require_api_server)):
    """Restore base and adapter state, then synchronize samplers."""
    return await server.submit_reset_zorl_session_async(request)
