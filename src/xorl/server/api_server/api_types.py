...
class UpdateDedicatedEndpointRequest(BaseModel):
    ...
    together_model_id: Optional[str] = Field(
        default=None, description="Optional Together model ID (auto-generated with timestamp if not provided)"
    )
...
class UpdateServerlessWeightsRequest(BaseModel):
    ...
    hf_repo_prefix: str = Field(default="xorl-adapter", description="Prefix for HuggingFace repository names")
    private_repo: bool = Field(default=True, description="Whether to create private HuggingFace repository")
    wait_for_completion: bool = Field(default=True, description="Whether to wait for Together AI upload to complete")
