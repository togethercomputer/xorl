...
class TestPackingConcatCollator:
    ...
    def test_concatenation_and_flash_attn(self, mock_parallel_state, sample_packed_features, sample_features):
        ...
        expected_cu_seq_lens = torch.tensor([0, 3, 5, 10], dtype=torch.int32)
        assert torch.equal(batch["cu_seq_lens_q"], expected_cu_seq_lens)
        assert torch.equal(batch["cu_seq_lens_k"], expected_cu_seq_lens)
        assert batch["max_length_q"] == 5
        assert batch["max_length_k"] == 5
        ...
        assert torch.equal(batch_single["cu_seq_lens_q"], torch.tensor([0, 3], dtype=torch.int32))
        assert batch_single["max_length_q"] == 3

        self._assert_extra_fields_and_multiple_seqs()
        self._assert_sequence_side_fields_concatenate_and_pad()

    @patch("xorl.data.collators.packing_concat_collator.get_parallel_state")
    def _assert_extra_fields_and_multiple_seqs(self, mock_parallel_state):
    ...
    @patch("xorl.data.collators.packing_concat_collator.get_parallel_state")
    def _assert_sequence_side_fields_concatenate_and_pad(self, mock_parallel_state):
        mock_ps = Mock()
        mock_ps.cp_enabled = False
        mock_parallel_state.return_value = mock_ps
        ...
        self._assert_hidden_match_weights_are_padded_as_sequence_field()

    def _assert_hidden_match_weights_are_padded_as_sequence_field(self):
        collator = PackingConcatCollator(pad_to_multiple_of=4)
        features = [
            {
                "input_ids": torch.tensor([1, 2]),
                "position_ids": torch.tensor([0, 1]),
                "hidden_match_weights": torch.tensor([0.25, 0.5]),
            },
            {
                "input_ids": torch.tensor([3]),
                "position_ids": torch.tensor([0]),
                "hidden_match_weights": torch.tensor([0.75]),
            },
        ]

        batch = collator(features)

        assert batch["hidden_match_weights"].shape == (1, 4)
        torch.testing.assert_close(
