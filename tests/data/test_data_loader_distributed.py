...
class TestDistributedDataAlignment:
    ...
    def test_partitioning_micro_batches_sp_batch_size_and_drop_last(self, mock_sampler_cls, mock_parallel_state, fake_text_dataset):
        ...
        builder_dl = DataLoaderBuilder(dataset=small_dataset, micro_batch_size=2, gradient_accumulation_steps=2, drop_last=True)
...
class TestMicroBatchCollatorAndSequenceSharding:
    ...
    def test_order_preservation_ga_alignment_and_error(self):
        ...
        collator = MicroBatchCollator(micro_batch_size=2, gradient_accumulation_steps=3, internal_collator=SimpleCollator())
        ...
        collator2 = MicroBatchCollator(micro_batch_size=3, gradient_accumulation_steps=2, internal_collator=SimpleCollator())
        ...
        collator3 = MicroBatchCollator(micro_batch_size=4, gradient_accumulation_steps=2, internal_collator=SimpleCollator())
...
class TestEndToEndAndPackedSequences:
    ...
    def test_pipeline_epoch_consistency_and_packed_sequences(self, mock_ps_collator, mock_ps_loader, fake_packed_dataset, fake_text_dataset):
        ...
        builder = DataLoaderBuilder(
            dataset=fake_packed_dataset, micro_batch_size=2, gradient_accumulation_steps=2,
            num_workers=0, prefetch_factor=None, seed=42,
        )
        dataloader = builder.build(verbose=False)
        ...
        builder2 = DataLoaderBuilder(
            dataset=fake_text_dataset, micro_batch_size=4, gradient_accumulation_steps=1,
            num_workers=0, prefetch_factor=None, seed=42,
        )
        ...
        class PackedDataset(Dataset):
            ...
            def __getitem__(self, idx):
                return [
                    {"input_ids": torch.tensor([idx * 10, idx * 10 + 1, idx * 10 + 2]), "labels": torch.tensor([idx * 10, idx * 10 + 1, idx * 10 + 2]), "position_ids": torch.tensor([0, 1, 2]), "attention_mask": torch.ones(3, dtype=torch.long)},
                    {"input_ids": torch.tensor([idx * 10 + 3, idx * 10 + 4]), "labels": torch.tensor([idx * 10 + 3, idx * 10 + 4]), "position_ids": torch.tensor([0, 1]), "attention_mask": torch.ones(2, dtype=torch.long)},
                ]
        ...
        builder3 = DataLoaderBuilder(
            dataset=PackedDataset(), micro_batch_size=2, gradient_accumulation_steps=1,
        ...
        class SimplePacked(Dataset):
            def __len__(self):
                return 16
            def __getitem__(self, idx):
                return [{"input_ids": torch.tensor([idx, idx + 1]), "labels": torch.tensor([idx, idx + 1]), "position_ids": torch.tensor([0, 1]), "attention_mask": torch.ones(2, dtype=torch.long)}]
        ...
        for dp_rank in [0, 1]:
            ...
            mock_ps_collator.return_value = mock_ps
            mock_ps_loader.return_value = mock_ps
            builder4 = DataLoaderBuilder(dataset=SimplePacked(), micro_batch_size=2, gradient_accumulation_steps=1, num_workers=0, prefetch_factor=None, pad_to_multiple_of=1)
        ...
        class VariablePackedDataset(Dataset):
            ...
            def __getitem__(self, idx):
                if idx % 2 == 0:
                    ...
                    return [
                        {"input_ids": torch.tensor([idx]), "labels": torch.tensor([idx]), "position_ids": torch.tensor([0]), "attention_mask": torch.ones(1, dtype=torch.long)},
                        {"input_ids": torch.tensor([idx + 1, idx + 2]), "labels": torch.tensor([idx + 1, idx + 2]), "position_ids": torch.tensor([0, 1]), "attention_mask": torch.ones(2, dtype=torch.long)},
                        {"input_ids": torch.tensor([idx + 3]), "labels": torch.tensor([idx + 3]), "position_ids": torch.tensor([0]), "attention_mask": torch.ones(1, dtype=torch.long)},
                    ]
        ...
        mock_ps_collator.return_value = mock_ps
        mock_ps_loader.return_value = mock_ps
        builder5 = DataLoaderBuilder(dataset=VariablePackedDataset(), micro_batch_size=2, gradient_accumulation_steps=1, num_workers=0, prefetch_factor=None, pad_to_multiple_of=1)
