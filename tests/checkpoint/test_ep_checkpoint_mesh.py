...
def test_ep_checkpoint_mesh_restore_policy(monkeypatch):
    for mesh_dim_names in (
        ("_pp_ep", "ep"),
        ("_pp_ep", "ep_fsdp"),
        ("ep", "ep", "ep_fsdp"),
    ):
    ...
    # ModelState must route both legacy and PP-parent meshes through the named
    # checkpoint dimensions before restoring expert state.
    for parent_shape, parent_names in (
        ((8, 2), ("ep", "ep_fsdp")),
        ...
        with monkeypatch.context() as scoped_patch:
            scoped_patch.setattr(checkpointer, "_restore_ep_dim", fake_restore)
            result = state.get_state_dict_with_ep_dim({"experts.weight": original})

        assert result["experts.weight"] is restored

    # The production restore then constructs the expected two-dimensional
    # DTensor for each supported parent layout.
    for parent_shape, parent_names, checkpoint_shape in (
        ((8, 2), ("ep", "ep_fsdp"), (8, 2)),
        ((2, 8, 1), ("_pp_ep", "ep", "ep_fsdp"), (8, 1)),
    ):
        parent_mesh = _FakeDeviceMesh(parent_shape, parent_names)
        ...
        assert call["device_mesh"].shape == checkpoint_shape
        assert [placement.dim for placement in call["placements"]] == [0, 1]

    _assert_drop_ep_dim_uses_stage_local_expert_fsdp_dimension(monkeypatch)


def _assert_drop_ep_dim_uses_stage_local_expert_fsdp_dimension(monkeypatch):
