...
def test_sparse_mla_backward(batch, seqlen, heads, dim, seqlen_kv, topk):
    ...
    for name, ref_g, tl_g in [
        ...
        if ref_g is None or tl_g is None:
            ...
            continue
        # NaN check must come before compute_diff: when tl_g contains NaN,
        # compute_diff's `denom > 0` test is False and rel_diff defaults to 0,
        # silently masking the failure.
        assert not torch.isnan(tl_g).any(), f"{name} contains NaN"
...
@requires_cuda()
@requires_tilelang()
def test_sparse_mla_backward_deterministic_dkv():
    """The deterministic dKV path (XORL_DSV4_DETERMINISTIC_DKV=1) must match the
    ...
    o, lse = sparse_mqa_fwd_interface(q, kv, attn_sink, topk_idxs, sm_scale=sm_scale)
    do = torch.randn_like(o)

    prev = _os.environ.get("XORL_DSV4_DETERMINISTIC_DKV", None)
    try:
        _os.environ["XORL_DSV4_DETERMINISTIC_DKV"] = "0"
        dq_a, dkv_a, ds_a = sparse_mqa_bwd_interface(q, kv, attn_sink, o, do, topk_idxs, lse, sm_scale=sm_scale)
        _os.environ["XORL_DSV4_DETERMINISTIC_DKV"] = "1"
        dq_d, dkv_d, ds_d = sparse_mqa_bwd_interface(q, kv, attn_sink, o, do, topk_idxs, lse, sm_scale=sm_scale)
    finally:
        if prev is None:
            _os.environ.pop("XORL_DSV4_DETERMINISTIC_DKV", None)
        else:
            _os.environ["XORL_DSV4_DETERMINISTIC_DKV"] = prev

    for name, a, d in (("dQ", dq_a, dq_d), ("dKV", dkv_a, dkv_d), ("dAttnSink", ds_a, ds_d)):
        assert not torch.isnan(d).any(), f"{name} (deterministic) contains NaN"
        diff = compute_diff(a.float(), d.float())
        print_diff(f"{name} det vs atomic", diff)
        assert diff.rel_diff < 0.05, f"{name} det/atomic rel_diff too large: {diff.rel_diff:.2e}"
...
def test_sparse_mla_backward_partial_invalid_indices():
    ...
    assert not torch.isnan(tl_o).any(), "NaN in output with invalid indices"
    for name, ref_g, tl_g in [
        ("dQ", ref_dq, q_tl.grad),
        ("dKV", ref_dkv, kv_tl.grad),
        ("dAttnSink", ref_d_sink, sink_tl.grad),
    ]:
        assert not torch.isnan(tl_g).any(), f"{name} contains NaN with invalid indices"
        diff = compute_diff(ref_g.float(), tl_g.float())
        print_diff(name, diff)
