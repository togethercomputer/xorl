...
from xorl.ops.exact.rope_fp32_single_round import (
...
def test_fp32_single_round_admission_shape_backward_and_table_layout_contract():
    ...
    with pytest.raises(RuntimeError, match="requires fp32 cos/sin"):
        single_round_apply_rotary_pos_emb(q, q, cos, sin)

    for shape in ((2, 1, 8, 8), (3, 2, 12, 8), (1, 1, 16, 8)):
        _assert_fp32_single_round_shape_and_partial_rotary_backward(*shape)
    ...
    cos_flat, sin_flat = build_single_round_cos_sin(cos, sin)
...
def _assert_fp32_single_round_shape_and_partial_rotary_backward(q_heads, k_heads, head_dim, rotary_dim):
    ...
    q_out, k_out = single_round_apply_rotary_pos_emb(q, k, cos, sin)
