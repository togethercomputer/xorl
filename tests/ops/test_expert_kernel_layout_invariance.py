...
def test_subset_composition_invariance(fixture):
    ...
    assert torch.equal(
        _active_slot_bits(sub_slots, ids[subset]), _active_slot_bits(full_slots[subset], ids[subset])
...
def test_m_sweep_row_stability(fixture):
    ...
    for extra in (1, 3, 17, 64, 129):
        ...
        assert torch.equal(
            _active_slot_bits(grown, base_i[:probe]), _active_slot_bits(reference, base_i[:probe])
