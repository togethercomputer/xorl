...
# MoE kernels require CUDA.  Skip unsupported hosts before entering the test
# body, but let failures importing XORL's own kernel modules surface normally.
pytestmark = [
    pytest.mark.gpu,
...
class TestExpertHistogramAndIndex:
    ...
    def _assert_histogram_and_index_compute(self):
...
class TestDeterministicScatter:
    """Run-invariant permutation from ``moe_index_compute``."""
    ...
    def _assert_deterministic_scatter(self):
        """Run-invariance, full slot coverage, and exact stable order."""
        from xorl.ops.group_gemm.kernel.moe import expert_histogram, moe_index_compute  # noqa: PLC0415
        ...
        # Validity: full slot coverage and each expert owns its cumsum region.
        assert det_a.shape == experts.shape
        assert det_a.dtype == torch.int64
        full = torch.arange(M * topk, device=det_a.device)
        assert torch.equal(det_a.flatten().sort()[0], full)
        for e in range(num_experts):
            mask = experts == e
            start = 0 if e == 0 else int(cumsum[e - 1])
            stop = int(cumsum[e])
            assert torch.equal(det_a[mask].sort()[0], torch.arange(start, stop, device=det_a.device))
...
class TestMoEGatherScatterAddGather:
    ...
    def test_gather_scatter_add_gather(self):
        ...
        TestExpertHistogramAndIndex()._assert_histogram_and_index_compute()
        TestDeterministicScatter()._assert_deterministic_scatter()
        from xorl.ops.group_gemm.kernel.moe import moe_add_gather, moe_gather, moe_scatter  # noqa: PLC0415

        # Gather across multiple hidden-dimension blocks.
        M3, topk3, N3 = 65, 2, 2049
        ...
        # Add-gather against an independent reference.
        M5, topk5, N5 = 8, 2, 16
        ...
        assert torch.allclose(
            ...
            naive_moe_add_gather(x5, y5, idx5),
            rtol=1e-3,
            atol=1e-3,
        )
