"""Scoped batch-invariant trunk-linear contract (XORL_BI_TRUNK_LINEAR, XORL-244).
...
therefore to serving) while backward stays on cuBLAS. These tests lock:
  ...
  - the XORL-244 loud-fail: the global interpose raises on grad-requiring inputs
...
# --------------------------------------------------------------------------- #
# XORL-244: the global interpose loud-fails on training forwards
# --------------------------------------------------------------------------- #
@requires_cuda
@pytest.mark.gpu
...
def test_global_interpose_silent_before_now_loud():
    # Regression pin for the bug XORL-244 fixes: without the guard, the aten::rms_norm
