...
from xorl.ops.loss import TokenPartial, policy_loss_function
...
def inputs():
    ...
    # Non-uniform mask across rows so each mb has a different valid count.
    ...
    return {
        ...
        "rollout_logprobs": torch.randn(B, S) * 0.3 - 1.5,
...
def _call(d, slc, *, loss_reducer=None, metric_reducer=None, **kwargs):
    return policy_loss_function(
        ...
        rollout_logprobs=d["rollout_logprobs"][slc],
...
@pytest.mark.parametrize(
    "extra",
    [
        pytest.param({}, id="vanilla_ppo"),
        pytest.param({"use_tis": True}, id="tis"),
        pytest.param({"icepop_beta": 1.5}, id="icepop"),
        pytest.param({"compute_kl_stats": True}, id="kl_stats"),
    ],
)
def test_identity_against_legacy(inputs, extra):
    """``TokenPartial(scale=mask.sum())`` reproduces the legacy local-mean result."""
    d = inputs
    ...
    for key, expected in legacy.metrics.items():
        ...
        # Ratio min/max are local reductions — identical regardless of reducer.
        # Mean metrics match because TokenPartial(scale=mask.sum()) ≡ legacy mean.
        assert_close(
...
def test_microbatch_composition(inputs, extra):
    """Sum of partial shares across non-uniform mbs equals the single-batch value."""
    d = inputs
    B = d["B"]
    ...
    # Reducer-routed metrics should compose under sum.
    composing = {
        "pg_clipfrac",
        "icepop_maskfrac",
        "tis_mean",
