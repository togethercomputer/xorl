...
def test_inject_fp8_training_moe_policy():
    model = TinyMoEModel()
    ...
    _assert_injection_preserves_per_expert_biases()
    _assert_summary_reports_unused_moe_modules()
...
def _assert_summary_reports_unused_moe_modules():
    model = TinyMoEModel()
...
def _assert_quack_moe_tp_fp8_single_rank_train_step_updates_master_weights(monkeypatch):
...
def test_fp8_grouped_forward_and_wgrad_policy(monkeypatch):
    torch.manual_seed(0)
    ...
    _assert_triton_grouped_same_nk_matches_reference_policy()
    ...
    with monkeypatch.context() as case_patch:
        _assert_fp8_grouped_wgrad_policy(case_patch)
    _assert_fp8_scalar_quack_grouped_policy()


def _assert_fp8_triton_grouped_same_nk_matches_reference(lengths, k, n):
...
def _assert_triton_grouped_same_nk_matches_reference_policy():
    for lengths, k, n in (
        ([4, 4], 128, 64),
        ([0, 129, 131], 257, 160),
    ):
        _assert_fp8_triton_grouped_same_nk_matches_reference(lengths, k, n)


def _assert_triton_grouped_helpers_honor_non_default_block_size():
    torch.manual_seed(0)
    lengths = [3, 5]
    cumsum = torch.tensor(lengths, device="cuda", dtype=torch.int32).cumsum(0)
    total_tokens = int(cumsum[-1].item())
...
def _assert_fp8_grouped_wgrad_policy(monkeypatch):
    torch.manual_seed(0)
    cumsum = torch.tensor([4, 8], device="cuda", dtype=torch.int32)
    a = (torch.randn(8, 128, device="cuda", dtype=torch.bfloat16) * 0.25).contiguous()
    ...
    _assert_scalar_quack_same_mn_dispatches_to_block_loop(monkeypatch)
    _assert_triton_grouped_wgrad_uses_precomputed_cu_seqlens()
    _assert_triton_grouped_wgrad_matches_reference_policy()


def _assert_scalar_quack_same_mn_dispatches_to_block_loop(monkeypatch):
...
def _assert_fp8_triton_grouped_wgrad_matches_reference(lengths, m, n):
...
def _assert_triton_grouped_wgrad_matches_reference_policy():
    for lengths, m, n in (
        ([4, 4], 128, 64),
        ([0, 0, 0], 64, 128),
        ([0, 129, 131], 96, 160),
    ):
        _assert_fp8_triton_grouped_wgrad_matches_reference(lengths, m, n)
...
def _assert_fp8_scalar_quack_grouped_policy():
    ...
    _assert_scalar_quack_uses_cu_seqlens_for_per_expert_scales()


def _assert_scalar_quack_uses_cu_seqlens_for_per_expert_scales():
    torch.manual_seed(0)
    lengths = [2, 6]
    cumsum = torch.tensor(lengths, device="cuda", dtype=torch.int32).cumsum(0)
    bogus_cumsum = torch.zeros_like(cumsum)
...
def _assert_moe_experts_quack_fp8_train_step_updates_master_weights(fp8_grouped_backend):
    torch.manual_seed(0)
...
@pytest.mark.gpu
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_moe_experts_quack_fp8_train_step_policy(monkeypatch):
    for fp8_grouped_backend in ("triton_grouped", "scalar_quack"):
        _assert_moe_experts_quack_fp8_train_step_updates_master_weights(fp8_grouped_backend)

    _assert_moe_train_step_with_biases_and_clamped_swiglu()
    with monkeypatch.context() as tp_patch:
        _assert_quack_moe_tp_fp8_single_rank_train_step_updates_master_weights(tp_patch)
    _assert_injected_dense_and_moe_fp8_model_train_step_updates_master_weights()


def _assert_moe_train_step_with_biases_and_clamped_swiglu():
    torch.manual_seed(0)
    experts = MoEExperts(num_experts=2, hidden_dim=128, intermediate_size=128, moe_implementation="quack")
