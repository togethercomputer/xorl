...
def test_block_fp8_gemm_auto_fallback_warns_once(monkeypatch):
    ...
    module = importlib.import_module("xorl.ops.quantize.block_fp8_quantize")
    a = torch.randn(16, 128, device="cuda", dtype=torch.float32).contiguous()
    b = torch.randn(16, 128, device="cuda", dtype=torch.float32).contiguous()
    a_fp8, a_scales = block_fp8_quantize(a, block_size=128)
    b_fp8, b_scales = block_fp8_quantize_gkn_rowwise(b, block_size=128)
    monkeypatch.setattr(module, "_SCALED_MM_AUTO_FALLBACK_WARNED", False)

    def fail_scaled_mm(*args, **kwargs):
        raise RuntimeError("simulated scaled-mm failure")

    monkeypatch.setattr(module, "_block_fp8_gemm_torch_scaled_mm", fail_scaled_mm)

    with pytest.warns(RuntimeWarning, match="falling back to Triton"):
        first = block_fp8_gemm(
            a_fp8,
            a_scales,
            b_fp8,
            b_scales,
