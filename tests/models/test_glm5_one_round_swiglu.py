...
from xorl.models.auto import _validate_canonical_glm52_model_scope, build_foundation_model
...
def test_glm52_exact_family_is_the_stamp_key() -> None:
    ...
    assert resolve_exact_contract_family(SimpleNamespace(_glm52_fullparam_training=True)) == EXACT_CONTRACT_FAMILY_GLM52


@pytest.mark.cpu
def test_fullparam_production_resolution_stamps_the_neutral_exact_family(monkeypatch) -> None:
    """The production resolver owns every exact-family stamp.

    Seed conflicting stale attributes as a checkpoint/config object might
    carry, then prove that the explicit full-param request replaces them with
    one coherent family before the model loader observes the config.
    """

    config = _official_glm52_config()
    config._glm52_block_fp8_qlora = True
    config._glm52_fullparam_training = False
    config._glm52_exact_contract = True
    config._qwen35_exact_contract = True
    config._exact_contract_family = "qwen3_5_moe"
    config._exact_one_round_swiglu = False
    ...
    monkeypatch.setattr("xorl.models.auto.get_parallel_state", lambda: parallel_state)
    ...
    assert config._glm52_fullparam_training is True
    assert config._glm52_block_fp8_qlora is False
    assert config._glm52_exact_contract is False
    assert config._qwen35_exact_contract is False
    ...
    assert config._exact_contract_family == EXACT_CONTRACT_FAMILY_GLM52
    assert config._exact_one_round_swiglu is True
