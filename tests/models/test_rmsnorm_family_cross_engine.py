...
against silent family flips in either engine (the 2026-07-04 norm-seed
incident and its inverse).
...
def test_trunk_contract_lane_qk_norm_site_class_bitwise(shape):
    ...
    the inverse of the norm-seed incident (a qk-norm family flip) is what this
    gate would catch."""
    x = _make(shape, 0)
    w = _make((shape[-1],), 300)
    norm = _xorl_module(shape[-1], RMS_NORM_FAMILY_NO_RESIDUAL, w)
