...
@pytest.mark.cpu
def test_canonical_trainer_admits_world16_dp1_without_changing_ep16_cp16():
    assert (16, 1, 1, 1) in _GLM52_CANONICAL_TRAINER_TOPOLOGIES
    assert (16, 1, 1, 2) in _GLM52_CANONICAL_TRAINER_TOPOLOGIES
    assert (32, 1, 1, 2) in _GLM52_CANONICAL_TRAINER_TOPOLOGIES
...
def test_index_share_context_identity_survives_fsdp_tensor_transform():
    ...
    with manager.invocation() as context:
        producer_context = _apply_to_tensors(lambda tensor: tensor, {"context": context})["context"]
        assert producer_context is context
        producer_context.publish(layer_index=0, layer_plan=plan, indices=payload)

        consumer_context = _apply_to_tensors(lambda tensor: tensor, {"context": context})["context"]
...
def test_index_share_survives_fsdp_cast_across_dense_producer_and_shared_consumer():
    ...
    def emulate_fsdp_mixed_precision_input_cast(_module, args, kwargs):
        ...
        transformed = _apply_to_tensors(cast, kwargs)
...
@pytest.mark.cpu
def test_sparse_selector_applies_hadamard_before_fp8_quantization():
    torch.manual_seed(1)
    ...
    assert selected.logical_indices.values.tolist() == [
        [[0, 5, 27, 30, 50, 62, 71, 72, 73, 75, 76, 81, 99, 104, 109, 111]]
    ]
...
@pytest.mark.cpu
def test_glm52_sparse_selector_loader_requires_matching_public_contract(monkeypatch):
    def selector(scores, lengths, topk):
        del lengths
        return torch.empty((scores.shape[0], topk), dtype=torch.int32)

    module = SimpleNamespace(select_canonical_logical_topk=selector)
    ...
    module.GLM52_SELECTOR_CONTRACT_VERSION = "incompatible"
    with pytest.raises(RuntimeError, match="contract mismatch"):
        sparse_selector_module._load_sparse_selection()

    module.GLM52_SELECTOR_CONTRACT_VERSION = GLM52_SELECTOR_VERSION
...
def test_canonical_moe_transport_is_an_explicit_model_mode():
    ...
    config.canonical_moe_transport = CanonicalMoETransport.CP_SHARDED_V3.value
    assert Glm5MoEBlock(config, layer_idx=1).canonical_moe_transport is CanonicalMoETransport.CP_SHARDED_V3
...
def _bind_semantic_canonicalizers(model, boundaries, *, serving: bool, skip_layer: int | None = None):
    plan_hash = ParallelPlan.glm52_trainer().digest
    for layer_id, layer in enumerate(model.model.layers):
        ...
        def canonical_forward(
            ...
            *,
            ...
            self.last_canonical_receipt = CanonicalMoEReceipt(
                contract_version=CANONICAL_MOE_REDUCE_VERSION,
                plan_hash=plan_hash,
                ...
                chunk_rows=rows,
                graph_mode=False,
                local_partial_policy="semantic_glm52_test_v1",
                engagement_key=f"semantic.layer.{_layer_id}.{self.canonical_engagement_count}",
            )
...
def test_semantic_moe_stack_boundary_logprob_engagement_permutation_and_composition(num_moe_layers, monkeypatch):
    ...
    assert len(trainer.model.last_forward_canonical_receipts) == num_moe_layers
    assert len({receipt.engagement_key for receipt in trainer.model.last_forward_canonical_receipts}) == num_moe_layers
