...
def _reference_slot_rows(hidden, ids, scales):
    ...
    return (hidden.index_select(0, tokens).float() * scales.index_select(0, experts).unsqueeze(1)).to(
        torch.bfloat16
    )
...
def _run_case(case: str, rank: int, world: int, device, buffer) -> None:
    ...
    # --- real transports 1 + 2 ---
    recv_x, recv_topk_idx, recv_topk_weights, recv_counts, _handle = dispatch_primary(
        buffer, hidden, weights, ids, _E
    )
    ...
    payload_rows = (slot_inputs.float() * scales.index_select(0, global_experts).unsqueeze(1)).to(
        torch.bfloat16
    )
    ...
    for src in range(world):
        ...
        kk = ann_ks[mask]
        grad_expected[mask] = (
            src_upstream.index_select(0, tok).float() * src_weights[tok, kk].unsqueeze(1)
        ).to(torch.bfloat16)
    ...
    recv_meta2, recv_slot_idx2, recv_slot_weights2, plan2 = dispatch_slot_level(
        buffer, hidden, weights, ids, _E
    )
    ...
    if not derived_arm_admitted:
        ...
        try:
            crosscheck_primary_source_indices(
                primary_handle=_handle, slot_handle=plan2.handle, view=view2, topk=_K
            )
        except DeepEPReceiveAlignmentError as refusal:
            assert "intranode-only" in str(refusal), refusal
        else:
            raise AssertionError(
                f"rank {rank} {case}: derived-receipt crosscheck did not refuse an internode handle"
            )
        if rank == 0:
            print(
...
def _run_ll_comparison(rank: int, world: int, device) -> None:
    ...
    print(f"[ll-compare] rank={rank} bitwise_match={matches} mismatched_elements={mismatched}/{combined_ll.numel()}", flush=True)
    assert matches, (
