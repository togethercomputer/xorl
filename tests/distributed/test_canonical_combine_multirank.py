...
def _build_shared_problem():
    ...
    table = (torch.randn((slot_tokens.numel(), _H), generator=generator, dtype=torch.float32)).to(torch.bfloat16)
...
def _combine_owned_slice(rank, metadata, delivered_rows, delivered_gids, transport_id):
    ...
    local_gids = torch.nonzero(owned, as_tuple=False).reshape(-1).unsqueeze(1) * _K + torch.arange(_K).unsqueeze(0)
    ...
    assert float(rel.max()) < 1e-2, f"rank {rank} {transport_id}: forward not numerically close to the oracle"
    assert torch.equal(
