...
def _slot_major_contribution_table(
    ...
    slot_tokens = (
        torch.arange(metadata.num_tokens, device=device).unsqueeze(1).expand_as(active)[active]
    )
    ...
    table = (
        token_inputs.to(device).index_select(0, slot_tokens)
        * expert_mats.to(device).index_select(0, slot_experts)
    ).to(torch.bfloat16)
...
def transport_slot_major(metadata, table, slot_tokens, slot_experts, *, pad_value: float):
    ...
    receipt = _receipt_from_placement(metadata, placement, num_rows, slot_tokens, slot_experts, "sim_slot_major_compact")
...
def _run_all_transports(device: torch.device, backend: str, pad_value: float, with_grads: bool):
    ...
    for name, build in _TRANSPORTS.items():
        ...
        if with_grads:
            ...
            metadata_i = CanonicalRouteMetadata(
                topk_ids=metadata.topk_ids, topk_weights=weights, num_experts=_E
            )
        else:
            metadata_i = metadata
...
def _assert_transport_equality(results: dict, *, with_grads: bool) -> None:
    ...
    for name, entry in results.items():
        ...
        if with_grads:
            ...
            assert torch.equal(
                entry["grad_weights"].view(torch.int32), reference["grad_weights"].view(torch.int32)
...
def test_slot_reorder_is_a_different_program():
    ...
    for seed in range(6):
        ...
        weights, ids = torch.topk(scores.softmax(dim=-1), k=_K, dim=-1)
        metadata = CanonicalRouteMetadata(
            topk_ids=ids.to(torch.int32), topk_weights=weights, num_experts=_E
        )
        active = metadata.active_mask
        ...
        table = (
            torch.randn((slot_tokens.numel(), hidden), generator=generator) * (2.0**exponents)
        ).to(torch.bfloat16)
    ...
    print(
        f"slot-reorder byte flip rate: {flipped_elements}/{total_elements} "
        f"({flipped_elements / total_elements:.2e})"
    )
...
def test_backward_matches_independent_dense_autograd_oracle():
    ...
    for name, build in _TRANSPORTS.items():
        ...
        oracle_out, oracle_grad_rows, oracle_grad_weights = _dense_autograd_oracle(
            rows, metadata, receipt, upstream
        )
        ...
        assert torch.equal(
            weights_custom.grad.view(torch.int32), oracle_grad_weights.view(torch.int32)
        ), f"{name}: d(topk_weights) FP32 bytes vs oracle"
        assert torch.equal(
            _bits(rows_custom.grad + 0.0), _bits(oracle_grad_rows + 0.0)
