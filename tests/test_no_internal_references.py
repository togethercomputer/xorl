...
The branch-name, tracker-ticket and assistant-name patterns cover identifiers
that resolve only inside the private tree: a development branch nobody outside
can fetch, an issue key nobody outside can open, and the name of an authoring
tool, which is attribution rather than technical content. The branch pattern is
deliberately bare rather than requiring a path suffix, because the leak this
caught was a prose mention of the branch on its own.
"""
