...
def test_bootstrap_passes_ep_intranode_to_parallel_state(monkeypatch, tmp_path):
    ...
    args = SimpleNamespace(train=train_args, model=SimpleNamespace(lm_head_fp32=True))
    ...
    assert trainer._causallm_loss_params["lm_head_fp32"] is True


def test_bootstrap_omits_lm_head_fp32_for_quack_linear(monkeypatch, tmp_path):
    train_args = SimpleNamespace(
        local_rank=0,
        global_rank=0,
        world_size=1,
    ...
    monkeypatch.setattr(trainer_module, "asdict", lambda _: {"train": {}})
    monkeypatch.setattr(trainer_module, "get_torch_device", lambda: SimpleNamespace(set_device=lambda *_: None))
    monkeypatch.setattr(trainer_module, "get_device_type", lambda: "cuda")
    monkeypatch.setattr(trainer_module, "get_nccl_backend", lambda: "nccl")
    monkeypatch.setattr(trainer_module.dist, "init_process_group", lambda *_, **__: None)
    monkeypatch.setattr(trainer_module.helper, "set_seed", lambda *_, **__: None)
    ...
    monkeypatch.setattr(trainer_module, "build_checkpointer", lambda *_, **__: "checkpointer")
    monkeypatch.setattr(trainer_module, "get_cpu_world_group", lambda: None)
    monkeypatch.setattr(trainer_module, "get_parallel_state", lambda: SimpleNamespace(device_mesh=None, ep_size=1))
    monkeypatch.setattr(trainer_module, "init_parallel_state", lambda **_: None)

    trainer._bootstrap()

    assert "lm_head_fp32" not in trainer._causallm_loss_params
