...
def _exact_glm52_config() -> Glm5Config:
    ...
    # The model builder installs this private admission marker after validating
    # the official geometry and before resolving the exact numerical program.
...
def test_rope_class_b_selection_policy(monkeypatch):
    ...
    _assert_class_b_selector_can_be_reset()
    _assert_canonical_glm_resolves_native_class_b(monkeypatch)
    _assert_canonical_glm_rejects_explicit_class_b_opt_out()
    _assert_non_glm_rope_defaults_and_opt_in_are_unchanged()
    _assert_canonical_glm52_model_and_numerical_program_policy(monkeypatch)


def _assert_class_b_selector_can_be_reset():
...
def _assert_canonical_glm_resolves_native_class_b(monkeypatch):
...
def _assert_canonical_glm_rejects_explicit_class_b_opt_out():
    config = _exact_glm52_config()
    for name in ("rope_native", "rope_class_b"):
...
def _assert_non_glm_rope_defaults_and_opt_in_are_unchanged():
...
def _assert_canonical_glm52_model_and_numerical_program_policy(monkeypatch):
    for name in (
    ...
    _assert_canonical_glm_rejects_incompatible_numerical_overrides()
    _assert_canonical_glm_rejects_incompatible_ce_override()
    _assert_non_glm_numerical_defaults_are_preserved()
    _assert_exact_glm52_model_scope_accepts_only_official_geometry()


def _assert_canonical_glm_rejects_incompatible_numerical_overrides():
    config = _exact_glm52_config()
...
def _assert_canonical_glm_rejects_incompatible_ce_override():
    config = _exact_glm52_config()
    with pytest.raises(ValueError, match="requires ce_mode='bi_fused'"):
        resolve_cross_entropy_mode(config, "compiled")


def _assert_non_glm_numerical_defaults_are_preserved():
    config = PretrainedConfig()
    program = resolve_model_numerical_program(
        config,
        attn_implementation=None,
...
def test_exact_qwen35_training_program_admission_policy():
    for config_factory in (_exact_qwen35_dense_config, _exact_qwen35_moe_config):
    ...
    _assert_non_qwen_rejects_qwen35_rmsnorm_v2()
    _assert_exact_qwen35_rejects_incompatible_numerical_overrides()
    _assert_exact_qwen35_rejects_incompatible_ce_override()
    _assert_exact_qwen35_moe_program_admission()
    _assert_exact_qwen35_topology_admission_policy()
    _assert_exact_qwen35_model_scope_admission_policy()


def _assert_non_qwen_rejects_qwen35_rmsnorm_v2():
...
def _assert_exact_qwen35_rejects_incompatible_numerical_overrides():
...
def _assert_exact_qwen35_rejects_incompatible_ce_override():
    config = _exact_qwen35_moe_config()
    config._qwen35_exact_contract = True
    with pytest.raises(ValueError, match="requires ce_mode='bi_fused'"):
        resolve_cross_entropy_mode(config, "compiled")
...
def _assert_exact_qwen35_moe_program_admission():
    config = _exact_qwen35_moe_config()
    config._qwen35_exact_contract = True
    _validate_exact_qwen35_moe_program(
        config,
...
def _assert_exact_qwen35_topology_admission_policy():
    cases = [
        (_exact_qwen35_moe_config, _qwen35_topology()),
        (
            _exact_qwen35_moe_config,
            _qwen35_topology(
                world_size=8,
                dp_size=8,
                dp_replicate_size=1,
    ...
    for config_factory, topology in cases:
        config = config_factory()
        config._qwen35_exact_contract = True
        _validate_exact_qwen35_topology(config, topology)

    _assert_exact_qwen35_rejects_noncertified_topologies()


def _assert_exact_qwen35_rejects_noncertified_topologies():
    config = _exact_qwen35_moe_config()
    config._qwen35_exact_contract = True
    for override in (
        {"world_size": 8},
...
def _assert_exact_glm52_model_scope_accepts_only_official_geometry():
...
def _assert_exact_qwen35_model_scope_admission_policy():
    for config_factory in (_exact_qwen35_dense_config, _exact_qwen35_moe_config):
        config = config_factory()
        config._qwen35_exact_contract = True
    ...
    _assert_exact_qwen35_rejects_wrong_layer_types()
    _assert_exact_qwen35_rejects_nearby_geometry()


def _assert_exact_qwen35_rejects_wrong_layer_types():
    config = _exact_qwen35_moe_config()
...
def _assert_exact_qwen35_rejects_nearby_geometry():
    for config_factory in (_exact_qwen35_dense_config, _exact_qwen35_moe_config):
        config = config_factory()
