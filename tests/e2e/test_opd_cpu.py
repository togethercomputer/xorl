...
from tests._helpers.opd import (
    FakeMooncakeClient,
    make_mooncake_hidden_cache,
    make_teacher_files,
    reference_grouped_opd_loss,
)
from xorl.distillation import MooncakeHiddenStore, TeacherActivationCache, TeacherHeadManager
...
class OPDCPUBackend(DummyBackend):
    def __init__(self, student_hidden_table: torch.Tensor, student_head: torch.Tensor, mooncake_store=None):
        super().__init__()
    ...
    async def forward_backward(
        ...
        hidden_cache = TeacherActivationCache(
            params["teacher_hidden_caches"], mooncake_store=self.mooncake_store, enable_async=False
        )
...
def test_opd_request_processor_to_backend_e2e(tmp_path):
    ...
    hidden_cache_metadata = make_mooncake_hidden_cache(store, teacher_hidden_caches)

    backend = OPDCPUBackend(student_hidden_table=student_hidden_table, student_head=student_head, mooncake_store=store)
    ...
    request = OrchestratorRequest(
        ...
        payload=ModelPassData(
            ...
            loss_fn_params={
                ...
                "teacher_hidden_caches": hidden_cache_metadata,
...
class _GlobalNormOPDBackend(DummyBackend):
    ...
    ranks", per the project loss-normalization contract), unlike the per-row
    normalization in OPDCPUBackend. With
