...
_skip_if_launcher_refactored = pytest.mark.skipif(
    ...
    reason="launcher refactor removed _master_addr_is_local / override-forwarding / "
...
def test_launch_forwards_model_path_when_overrides_empty(tmp_path):
    launcher = _make_launcher(
        model_path="/models/cache/.../snapshots/abc",
        tokenizer_path="/models/cache/.../snapshots/abc",
        server_overrides={},
        config_path=str(_write_flat_config(tmp_path)),
    )
    cmd = _capture_torchrun_cmd(launcher)
    assert "--model_path=/models/cache/.../snapshots/abc" in cmd
    assert "--tokenizer_path=/models/cache/.../snapshots/abc" in cmd
