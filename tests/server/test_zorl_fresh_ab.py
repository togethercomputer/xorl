...
def test_invalid_materialization_does_not_activate_generation():
    runner = ModelRunner.__new__(ModelRunner)
    state = ZORLSessionState.from_session_spec("default", _session_spec("fresh_ab"))
    assert state is not None
    runner._zorl_sessions = {"default": state}
    runner._adapter_manager = object()
    runner._lora_session_specs = {"default": {}}
    runner.ensure_lora_adapter = lambda *args, **kwargs: None

    with pytest.raises(ValueError, match="num_shards"):
        runner.start_zorl_generation(
            "default",
            materialization={"mode": "pair_shard", "num_shards": 0, "shard_index": 0},
        )
