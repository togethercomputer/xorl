...
def _parallel_state(**overrides):
    return SimpleNamespace(
        ...
        dp_rank=overrides.get("dp_rank"),
        dp_size=overrides.get("dp_size"),
        dp_replicate_rank=overrides.get("dp_replicate_rank"),
        dp_replicate_size=overrides.get("dp_replicate_size"),
        dp_shard_size=overrides.get("dp_shard_size"),
...
def test_select_batches_shares_one_slice_across_fsdp_shard_ranks(monkeypatch):
    batches = [_batch(10)]
    ...
    for rank in range(4):
        ...
        monkeypatch.setattr(runner_dispatcher_module, "get_parallel_state", lambda state=state: state)

        my_batches, routed_experts, routed_logits = _dispatcher(rank=rank, world_size=4)._select_and_prepare_batches(
            batches,
            routed_experts=["r0"],
            routed_expert_logits=["l0"],
        )

        assert len(my_batches) == 1
        assert int(my_batches[0]["num_samples"]) == 1
        assert torch.equal(my_batches[0]["input_ids"], torch.tensor([[10, 11]]))
        assert routed_experts == ["r0"]
        assert routed_logits == ["l0"]


def test_select_batches_slices_by_replicate_rank_not_fsdp_shard_rank(monkeypatch):
    batches = [_batch(10), _batch(20)]
    ...
    for rank, replicate_rank, expected_batch_id, expected_routed, expected_logits in cases:
        state = _parallel_state(
            dp_rank=rank,
            dp_size=4,
            dp_replicate_rank=replicate_rank,
            dp_replicate_size=2,
            dp_shard_size=2,
        )
        monkeypatch.setattr(runner_dispatcher_module, "get_parallel_state", lambda state=state: state)
        ...
        assert len(my_batches) == 1
        assert int(my_batches[0]["num_samples"]) == 1
        assert torch.equal(my_batches[0]["input_ids"], torch.tensor([[expected_batch_id, expected_batch_id + 1]]))
        assert routed_experts == [expected_routed]
        assert routed_logits == [expected_logits]


def test_select_batches_shares_one_slice_across_tp_ranks(monkeypatch):
    batches = [_batch(10), _batch(20), _batch(30), _batch(40)]

    for rank, expected_batch_id in [(0, 10), (1, 10), (2, 20), (3, 20), (6, 40), (7, 40)]:
        state = _parallel_state(tp_size=2, dp_rank=rank // 2, dp_size=4)
        monkeypatch.setattr(runner_dispatcher_module, "get_parallel_state", lambda state=state: state)

        my_batches, routed_experts, routed_logits = _dispatcher(rank=rank, world_size=8)._select_and_prepare_batches(
            batches,
            routed_experts=["r0", "r1", "r2", "r3"],
            routed_expert_logits=["l0", "l1", "l2", "l3"],
        )
        ...
        assert len(my_batches) == 1
        assert torch.equal(my_batches[0]["input_ids"], torch.tensor([[expected_batch_id, expected_batch_id + 1]]))
        assert torch.equal(my_batches[0]["labels"], torch.tensor([[expected_batch_id + 2, expected_batch_id + 3]]))
        assert routed_experts == [f"r{expected_batch_id // 10 - 1}"]
        assert routed_logits == [f"l{expected_batch_id // 10 - 1}"]
...
def test_select_batches_shares_one_slice_across_tp_ranks_under_ep(monkeypatch):
    state = _parallel_state(
        tp_size=2,
        ep_enabled=True,
        ep_size=4,
    ...
    for rank in (4, 5):
        ...
        assert len(my_batches) == 1
        assert torch.equal(my_batches[0]["input_ids"], torch.tensor([[30, 31]]))
        assert torch.equal(my_batches[0]["labels"], torch.tensor([[32, 33]]))
        assert routed_experts == ["r2"]
        assert routed_logits == ["l2"]
...
def test_microbatch_diagnostic_dump_includes_r3_payloads(tmp_path, monkeypatch):
    ...
    state = _parallel_state(ep_enabled=True, ep_size=8, dp_size=8)
    batch = {
