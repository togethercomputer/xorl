...
This wave also removes the standalone `xorl.rl` package and its five formula-repetition reports. Its six exported
Slime-style tensor helpers had no trainer, runner, loss, CLI, example, documentation, or other source caller. Integrated
loss suites continue to cover the actual policy, KL, importance-sampling, OPD, and Dr.GRPO paths used by training.
