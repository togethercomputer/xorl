...
XoRL supports single-stage-per-rank schedules and virtual-stage schedules. `pipeline_parallel_virtual_stages` is the number of model chunks owned by each PP rank.
...
### Virtual-stage and zero-bubble schedules

| Schedule | Stage placement | Virtual stages | Micro-batch constraint |
|---|---|---|---|
| `Interleaved1F1B` | Loop: rank `r` owns stages `r`, `r+PP`, ... | Use 2 or more model chunks per rank | Must satisfy the interleaved round-divisibility check |
| `InterleavedZeroBubble` | Loop | Use 2 or more model chunks per rank | Same round-divisibility check; backward is split into input- and weight-gradient phases |
| `ZBVZeroBubble` | V-shaped/zigzag | Exactly 2 | Backward is split into input- and weight-gradient phases |
| `DualPipeV` | V-shaped/zigzag | Exactly 2 | `n_microbatches >= PP × virtual_stages` |

Virtual stages can reduce pipeline bubbles, but they also create multiple model chunks per rank. They are currently incompatible with expert parallelism and with inference weight sync. Use `1F1B` or `GPipe` on those lanes.
...
All schedules are backed by `torch.distributed.pipelining` via `get_schedule_class`.
...
This applies to one model chunk per rank. `pipeline_parallel_virtual_stages > 1`
with EP is rejected because EP-aware optimizer and state-dict plumbing currently
assumes one model part per rank.
...
| Constraint | Reason |
|---|---|
| Schedule-specific micro-batch count | `n_microbatches = gradient_accumulation_steps`. Interleaved schedules enforce round divisibility; `DualPipeV` requires at least `PP × virtual_stages`. GPipe and 1F1B do not impose the old universal `n_microbatches >= PP` validation rule. More micro-batches can still improve utilization. |
| `tie_word_embeddings: false` | `embed_tokens` (stage 0) and `lm_head` (last stage) cannot share weights across different GPUs. |
| Virtual PP restrictions | Virtual stages are supported by the interleaved and V-style schedules, but not with EP or inference weight sync. |
...
| `train.pipeline_parallel_schedule` | `str` | `"1F1B"` | `1F1B`, `GPipe`, `Interleaved1F1B`, `InterleavedZeroBubble`, `ZBVZeroBubble`, or `DualPipeV`. |
| `train.pipeline_parallel_virtual_stages` | `int` | `1` | Model chunks per PP rank. Use 1 for GPipe/1F1B, normally 2+ for interleaved schedules, and exactly 2 for ZBV/DualPipeV. |
| `train.pipeline_parallel_input_weight` | `int` | `1` | Layer-equivalent cost assigned to the embedding when balancing stages. |
| `train.pipeline_parallel_output_weight` | `int` | `1` | Layer-equivalent cost assigned to final norm, LM head, and CE when balancing stages. |
...
| `train.gradient_accumulation_steps` | `int` | required | Sets `n_microbatches`; constraints depend on the selected schedule. |
...
| `src/xorl/arguments.py` | Pipeline size, schedule, virtual-stage and stage-balancing fields, variable sequence lengths, and reshard behavior |

## Source

| File | Description |
|---|---|
| [`src/xorl/distributed/pipeline_parallel.py`](https://github.com/togethercomputer/xorl/blob/main/src/xorl/distributed/pipeline_parallel.py) | Stage construction, schedule validation/building, `_pp_forward`, and `build_pp_stage` |
