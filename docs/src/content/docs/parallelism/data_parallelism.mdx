...
train:
  ...
  gradient_accumulation_steps: 4  # extra micro-batches reduce the 1F1B bubble
  micro_batch_size: 1
...
When PP is enabled, `gradient_accumulation_steps` determines the number of micro-batches in the pipeline. The hard constraints depend on the selected schedule; for 1F1B, increasing the count relative to `pipeline_parallel_size` improves utilization:
...
| File | Description |
|---|---|
| [`src/xorl/distributed/torch_parallelize.py`](https://github.com/togethercomputer/xorl/blob/main/src/xorl/distributed/torch_parallelize.py) | `fully_shard` application, gradient divide factor, EP prefetch setup |
| [`src/xorl/distributed/parallel_state.py`](https://github.com/togethercomputer/xorl/blob/main/src/xorl/distributed/parallel_state.py) | Device mesh construction, DP/EP/TP group initialization |
