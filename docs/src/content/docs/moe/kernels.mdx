...
|---|---|---|---|---|---|
| `triton` | Triton `group_gemm_same_nk` | Yes | Partial† | Yes | Triton grouped GEMM; works with recompute paths |
| `quack` | CuTe DSL kernels | Yes | No | Yes | SM90/SM100 kernels; optional package availability is checked at runtime |
| `native` | `torch._grouped_mm` | Yes | Yes (compute path) | Yes | PyTorch grouped GEMM, without the Quack extension |
| `eager` | Per-expert PyTorch matmuls | Yes | Yes | Yes | Reference/debugging path; EP uses the registered eager EP compute function |

```yaml
model:
  moe_implementation: triton   # example; benchmark against native/quack for the target shape
```
...
The current registries expose these Quack variants. This table does not imply support for every dtype, architecture, quantization, or topology combination:
...
Quack handles expert FFN compute while DeepEP handles dispatch/combine, so they can be evaluated together on supported H100-class deployments. Their SM allocations compete, however; compare this pairing with Quack + AllToAll and with other expert backends on the same model and token shape.
...
- **Capability checks still apply** — LoRA and hybrid shared-LoRA paths are registered, but dtype, quantization, architecture, and dispatch combinations can impose narrower constraints
...
| Method | Backward recomputes | EP comm recomputed | Retained activation state |
|---|---|---|---|
| `recompute_full_layer` (default) | Entire decoder layer | **Yes** — dispatch/combine are replayed | lowest |
| `recompute_before_dispatch` | Attention + router only; retains dispatch + expert + combine | **No** | higher |
| `no_recompute` | Nothing | **No** | highest |

```yaml
# No-recompute candidate (use only after a shape-matched fit check):
train:
...
# Lowest retained-activation state (start here for memory-constrained runs):
train:
  enable_gradient_checkpointing: true
...
The retained tensors and avoided work differ by model and backend. Measure peak memory and end-to-end step time at the intended sequence length before changing the default.

### Why topology matters

`recompute_full_layer` replays dispatch and combine during backward; `recompute_before_dispatch` retains their outputs instead. Avoiding that replay can matter more when EP communication crosses nodes, but the result depends on fabric, token balance, overlap, retained-memory pressure, and expert-kernel cost. Treat the choice as a measured memory/throughput tradeoff rather than a fixed speedup.
