...
Ensure `moe_checkpoint_method: moe_act` is set — R3 routing replay is required for EP + gradient checkpointing. See [MoE Routing Replay](/moe/overview/#routing-replay-r3).
...
| File | Description |
|---|---|
| [`src/xorl/models/layers/moe/moe_block.py`](https://github.com/togethercomputer/xorl-internal/blob/main/src/xorl/models/layers/moe/moe_block.py) | `MoEBlock` — DeepEP dispatch/combine integration, async combine stream management |
| [`src/xorl/models/layers/moe/experts.py`](https://github.com/togethercomputer/xorl-internal/blob/main/src/xorl/models/layers/moe/experts.py) | `MoEExperts._ep_forward()` — DeepEP dispatch, compute, combine phases; XORL_DEBUG_EP timing |
