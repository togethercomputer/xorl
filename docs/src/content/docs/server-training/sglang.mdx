...
#   --rl-on-policy-target xorl --enable-fp32-lm-head --tp-size 1 \
#   --quantization fp8 --enable-lora --max-lora-rank 32 \
...
| Profile | Trainer | Receiver flags | Weights pushed per step |
|---|---|---|---|
| `full` | bf16 full-weight | base flags only | full weights (`sync_inference_method`) |
| `lora` | bf16 base + bf16 LoRA (`unfuse_for_lora=true`) | `--enable-lora --max-lora-rank R --lora-target-modules ...` | LoRA adapters (`/load_lora_adapter`) |
| `fp8_lora` | FP8 block-e4m3 frozen base (`block_fp8` QLoRA) + bf16 LoRA | `--quantization fp8 --enable-lora --max-lora-rank R --lora-target-modules ...` | LoRA adapters (`/load_lora_adapter`); base stays frozen |
