...
Cache invalidation follows the sync request. `cache_invalidation_mode: auto` resolves to a flush for FP8 weight sync into an FP8-KV endpoint; `flush` always requests one, and `none` relies on receiver versioning. A flush must be paired with `pause_mode: retract` so no running batch is retained. Use `pause_mode: in_place` only with `cache_invalidation_mode: none` and `flush_cache: false`. See the [weight-sync overview](/xorl/server-training/weight-sync/overview/) for the valid combinations.
