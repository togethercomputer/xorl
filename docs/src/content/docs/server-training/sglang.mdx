...
The xorl-sglang revision pinned by this checkout carries the XoRL integration. Its branch provenance changes while the coordinated PR stack is reviewed and rebased, so this page describes stable integration surfaces instead of assuming the pin matches `main` or quoting line/file counts.
