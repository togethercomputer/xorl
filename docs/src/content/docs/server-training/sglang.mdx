...
### Deriving launch flags from a train/serve profile

When the training server config sets
[`train_serve_profile`](/config-reference/server/#trainserve-profile)
(`full`, `lora`, or `fp8_lora`), the alignment-relevant launch flags are a
deterministic function of that config — print them instead of hand-copying:
...
#   --tp-size 1 --quantization fp8 --enable-lora --max-lora-rank 32 \
#   --lora-target-modules qkv_proj o_proj gate_up_proj down_proj
```

Capacity tuning (`--port`, `--mem-fraction-static`, CUDA-graph limits) stays
with the operator. Compatibility mapping per profile:

| Profile | Trainer | Receiver flags | Serving numerics | Weight sync |
|---|---|---|---|---|
| `full` | bf16 full-weight | `--rl-on-policy-target xorl --enable-fp32-lm-head` | XoRL exact (K3=0 contract) | full bf16 weights (NCCL `sync_inference_weights`) |
| `lora` | bf16 base + bf16 LoRA (`unfuse_for_lora=true`) | `--rl-on-policy-target xorl --enable-fp32-lm-head --enable-lora --max-lora-rank R --lora-target-modules ...` | XoRL exact (K3=0 contract) | merged bf16 weights (NCCL `sync_inference_weights`) |
| `fp8_lora` | FP8 block-e4m3 frozen base (`block_fp8` QLoRA) + bf16 LoRA | `--quantization fp8 --enable-lora --max-lora-rank R --lora-target-modules ...` | stock SGLang FP8 path — **no exact contract** | merged weights re-quantized to block-FP8 in flight (NCCL `sync_inference_weights`) |

The exact value program is a bf16 contract: a receiver launched with
`--rl-on-policy-target xorl` on quantized weights fails closed at boot, and
the trainer-side dense-Qwen3/Qwen3.5 exact stamps disengage under QLoRA
(`xorl.models.auto.qwen_exact_contracts_engaged`), so `fp8_lora` deliberately
derives the stock FP8 serving path and carries no bitwise parity guarantee.

`fp8_lora` quantizes the **base model**, never the adapter weights — the
adapters remain bf16 on both sides. Trainer LoRA targets SGLang has no name
for (e.g. Qwen3.5's GDN `g_proj`) map to `--lora-target-modules all`, a
superset; the pushed adapters still carry the concrete module set.

`/add_inference_endpoint` validates every registered receiver against the
profile (base quantization, `--enable-lora`, `max_lora_rank`) and rejects
mismatches before any weight sync happens.
