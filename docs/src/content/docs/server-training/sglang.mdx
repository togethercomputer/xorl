...
Reduction order is also architecture-owned. The canonical Qwen/GLM MoE program uses its balanced adjacent-pair fold, while DSV4-Flash retains the pinned NCCL contributor chain `(1, 2, 3, 4, 5, 6, 7, 0)`. Substituting one for the other changes BF16 association and requires a fresh cross-engine qualification; it is not a merge-time refactor.
...
The pinned receiver does not associate KV or prefix-cache entries with `weight_version`; that value is control-plane metadata only. Invalidate cached state after every online weight update that may be followed by cache reuse. Select `cache_invalidation_mode: flush` with `pause_mode: retract` (or `abort` when dropping requests is acceptable). Do not carry cached state across an update with `cache_invalidation_mode: none` or `pause_mode: in_place`. See the [weight-sync overview](/xorl/server-training/weight-sync/overview/) for the safe request.
...
| Flag | Description |
|---|---|
| `--rl-on-policy-target xorl` | Select deterministic inference and the architecture-owned XoRL numerical program when the model has one. Weight-sync endpoints are independent of this flag. Use `xorl-batch-invariant` for the generic batch-invariant target. |
