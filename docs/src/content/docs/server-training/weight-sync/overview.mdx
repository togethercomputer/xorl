...
LoRA sessions do not use weight sync. Adapters are published to inference
endpoints in PEFT format via SGLang's `/load_lora_adapter`, and endpoints for
a LoRA training server must support LoRA (`--enable-lora`, with
`--max-lora-rank` covering the substrate rank — registration rejects
endpoints that do not). Weight sync's NCCL transport is dense full-weight
only. See the [LoRA adapter docs](/adapters/lora/) for the publication flow.
