...
|---|---|---|
| XORL trainer | Qwen3.5 norm-family routing and tests; FP32 GDN beta; fixed q/k L2Norm `BT16/w8/s3` with config-sweep tests; opt-in GDN layer captures | fixed L2Norm and model routing are normal Qwen3.5 contract behavior; `XORL_FLA_L2NORM_AUTOTUNE=1` is the explicit non-contract opt-out |
