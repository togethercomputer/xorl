...
EP8 dispatch and `canonical_moe_fold_fp64_v3`. Transport restores logical
contributor order; the fold promotes leaves and evaluates every adjacent-pair
node in FP64 before one final low-precision output cast.
...
EP dispatch and the same 16-leaf logical FP64 adjacent-pair fold remain
