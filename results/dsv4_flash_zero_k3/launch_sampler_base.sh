...
# Lane-scoped JIT cache: the default ~/.cache/sglang is shared across hosts
# and races (observed: missing __triton_launcher .so killing schedulers
# mid-warmup). A lane-private weka path has exactly one writer and survives
# pod restarts.
export SGLANG_CACHE_DIR="$REPO/results/dsv4_flash_zero_k3/jit-cache"
