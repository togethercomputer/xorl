...
exec .venv/bin/python -m sglang.launch_server \
  ...
  --enable-dp-attention \
  --enable-lora \
  --lora-paths zero="$REPO/results/dsv4_flash_zero_k3/adapter_zero" nonzero="$REPO/results/dsv4_flash_zero_k3/adapter_nonzero" \
  --trust-remote-code \
  --debug-tensor-dump-output-folder "$REPO/results/dsv4_flash_zero_k3/dumps/sampler_nonzero" \
