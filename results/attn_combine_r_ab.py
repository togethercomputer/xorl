...
Builds two MKQwen3 models at Cfg(S=S) — one with MK_ATTN_COMBINE_R unset (current
default) and one with MK_ATTN_COMBINE_R=T — in the given construction order, checks
...
def build_model(S, band):
    if band:
        os.environ["MK_ATTN_COMBINE_R"] = band
    else:
