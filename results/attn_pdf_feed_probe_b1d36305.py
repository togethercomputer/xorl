...
MKDIR = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
    os.path.dirname(__file__), "..", "experiments", "fused-training-megakernel")
...
def gen_inputs(S, nq, nkv, D=64):
    ...
    p = mk.Program()
    p.instr(mk.OP_ATTN_FWD_WG, nq * (S // 128),
            [p.buf(qkv), p.buf(O), p.buf(LSE), S, nq, nkv, D, scale])
...
def run_op(ext, mode, op, ntiles, qkv, dO, LSE, Drow, S, nq, nkv, scale, C, stride):
    ...
    p = mk.Program()
    p.instr(op, ntiles,
            [p.buf(qkv), p.buf(dO), p.buf(LSE), p.buf(Drow), p.buf(ws),
             S, nq, nkv, 64, scale, C])
...
dq_off_c1 = run_op(EXT_OFF, "pdf", mk.OP_ATTN_DQ_WG, nq * nqt * 1,
                   qkv, dO, LSE, Drow, S, nq, nkv, scale, 1, stride)
dq_on_c1 = run_op(EXT_ON, "pdf", mk.OP_ATTN_DQ_WG, nq * nqt * 1,
                  qkv, dO, LSE, Drow, S, nq, nkv, scale, 1, stride)
fails += not bitwise("dq C=1 pdf feed-on vs feed-off", dq_on_c1, dq_off_c1)

dq_on_df = run_op(EXT_ON, "df", mk.OP_ATTN_DQ_WG, nq * nqt * 1,
                  qkv, dO, LSE, Drow, S, nq, nkv, scale, 1, stride)
fails += not bitwise("dq C=1 df feed-on(dormant) vs pdf feed-off", dq_on_df, dq_off_c1)

dq_off_c4 = run_op(EXT_OFF, "pdf", mk.OP_ATTN_DQ_WG, nq * nqt * 4,
                   qkv, dO, LSE, Drow, S, nq, nkv, scale, 4, stride)
dq_on_c4 = run_op(EXT_ON, "pdf", mk.OP_ATTN_DQ_WG, nq * nqt * 4,
                  qkv, dO, LSE, Drow, S, nq, nkv, scale, 4, stride)
...
dkv_off = run_op(EXT_OFF, "pdf", mk.OP_ATTN_DKV_WG, nkv * nqt * G * 1,
                 qkv, dO, LSE, Drow, S, nq, nkv, scale, 1, stride)
dkv_on = run_op(EXT_ON, "pdf", mk.OP_ATTN_DKV_WG, nkv * nqt * G * 1,
                qkv, dO, LSE, Drow, S, nq, nkv, scale, 1, stride)
...
dkv1_off = run_op(EXT_OFF, "pdf", mk.OP_ATTN_DKV_WG, nkv1 * nqt1 * 1 * 1,
                  qkv1, dO1, LSE1, Drow1, S1, nq1, nkv1, scale1, 1, stride1)
dkv1_on = run_op(EXT_ON, "pdf", mk.OP_ATTN_DKV_WG, nkv1 * nqt1 * 1 * 1,
                 qkv1, dO1, LSE1, Drow1, S1, nq1, nkv1, scale1, 1, stride1)
...
def run_both(ext, mode):
    ...
    p = mk.Program()
    p.instr(mk.OP_ATTN_DKV_WG, nkv * nqt * G,
            [p.buf(qkv), p.buf(dO), p.buf(LSE), p.buf(Drow), p.buf(wsq),
             S, nq, nkv, 64, scale, 1])
    p.instr(mk.OP_ATTN_DQ_WG, nq * nqt,
            [p.buf(qkv), p.buf(dO), p.buf(LSE), p.buf(Drow), p.buf(wsq),
             S, nq, nkv, 64, scale, 1])
    p.finalize().run(ext, mode=mode)
    torch.cuda.synchronize()
