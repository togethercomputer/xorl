#!/usr/bin/env python3
"""Host route/load proof for qwen NT top-level production-context SASS proof."""
...
class FakeExt:
    __file__ = "/tmp/fake-qwen-nt-topembed.so"
...
DEFINE = "-DMK_GEMM_N256_NT_SUPERTILE_TOPEMBED"
...
def emit(tag: str, cfg: Cfg, updates: dict[str, str | None]) -> dict[str, object]:
    ...
    print("QWEN_NT_TOPEMBED_ROUTE_JSON " + json.dumps(payload, sort_keys=True), flush=True)
    return payload
...
def main() -> None:
    ...
    same_fields(l2_default, l2_forced, invariant_fields, "l2 forced", errors)

    if l2_forced["n_instr"] != 78 or l2_forced["critical_path"] != 44:
        errors.append("unexpected qwen4b-l2 route shape")
    if l2_forced["gated"] != 14 or l2_forced["smem_bytes"] != 151552:
    ...
    for tag, row in (
        ("l1 negative", l1_forced),
        ("small negative", small_forced),
        ...
        if row["has_nttop_suffix"] or row["has_nttop_define"]:
            errors.append(f"{tag} unexpectedly enabled topembed")
    ...
    print("QWEN_NT_TOPEMBED_ROUTE_SUMMARY " + json.dumps(summary, sort_keys=True), flush=True)
    if errors:
        raise SystemExit(1)
